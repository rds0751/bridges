...
const {DEBRIDGEGATE_ADDRESS2} = process.env;
...
(async () => {
    try {
        ...
        const chainIdTo = '1337';
        const rpc = Web3RpcUrl[chainIdTo];
        const web3 = new Web3(rpc);

        const debridgeGateInstance = new web3.eth.Contract(DeBridgeGateJson.abi, DEBRIDGEGATE_ADDRESS2);
        const isSubmissionUsed = await debridgeGateInstance.methods.isSubmissionUsed(SUBMISSION_ID).call();
        logger.debug("working till here")
        if (isSubmissionUsed) {
            logger.error(`Submission already used`);
            return;
        }

        let mergedSignatures = '0xd810d7f7d67be44881d574dc0e4681313d82f5740da800b1ce3e9d8a602240902eab0c575b1a0a6c899d7701dd35b5823f4255e02463abefc9deff51726c1f031b';
        ...
        const autoParamsFrom = await _packSubmissionAutoParamsFrom(web3, "0x852e7627aEFF3ed6105F600D985cC6f74DBd6640", '0x');

        await claim(
            web3,
            debridgeGateInstance,
            "0x9805a0d9ee98ce5dae28a1799074e6121c863794c241beefd9dc1b0b083018e9",
            "0x3c20902e",
            "42",
